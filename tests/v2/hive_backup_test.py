...
from hive.main.v2.hive_backup import HiveBackup
...
from hive.util.constants import DID, HIVE_MODE_TEST, DID_INFO_DB_NAME, VAULT_ORDER_COL, VAULT_BACKUP_SERVICE_COL, \
    INTER_BACKUP_SAVE_FINISH_URL_V2, APP_ID, INTER_BACKUP_PATCH_HASH_URL_V2, \
    INTER_BACKUP_FILE_URL_V2, INTER_BACKUP_PATCH_DELTA_URL_V2, CHUNK_SIZE
...
from hive.util.payment.vault_service_manage import setup_vault_service, get_vault_path
from hive.util.pyrsync import rsyncdelta, rsyncdelta
from tests.v2 import test_common
from hive import settings
from tests.v2.test_common import upsert_collection, create_upload_file, prepare_vault_data, copy_to_backup_data, \
    move_to_backup_data
...
class HiveBackupTestCase(unittest.TestCase):
    ...
    def prepare_active_backup_hive_node(self):
        ...
        rt, s = self.parse_response(
            self.test_client.post(INTER_BACKUP_SAVE_FINISH_URL_V2,
    ...
    def delete_backup_file(self, file_name):
        ...
        r = self.test_client.post(INTER_BACKUP_FILE_URL_V2,
                                  json=param,
                                  headers=self.auth)
    ...
    def put_backup_file(self, file_name, data):
        ...
        put_file_url = INTER_BACKUP_FILE_URL_V2 + "?file=" + file_name
    ...
    def get_backup_file(self, file_name):
        get_file_url = INTER_BACKUP_FILE_URL_V2 + "?file=" + file_name
        r = self.test_client.get(get_file_url, headers=self.upload_auth)
        self.assert200(r.status_code)
        return r.get_data()

    def get_backup_file_hash(self, file_name):
        get_file_hash_url = INTER_BACKUP_PATCH_HASH_URL_V2 + "?file=" + file_name
    ...
    def post_backup_file_delta(self, file_name, delta):
        post_delta_url = INTER_BACKUP_PATCH_DELTA_URL_V2 + "?file=" + file_name
