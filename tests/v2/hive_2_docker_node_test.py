...
from hive.main.v2 import view
from hive.main.v2.hive_backup import HiveBackup
from hive.util.constants import INTER_BACKUP_FILE_URL_V2
...
class Hive2NodeTest(unittest.TestCase):
    ...
    def test_4_inter_put_file(self):
        ...
        temp.seek(0)
        temp.name = 'temp.txt'
        url = self.host2 + INTER_BACKUP_FILE_URL_V2 + '?file=' + file_name
        r = requests.put(url,
                         data=temp,
                         headers={"Authorization": "token " + backup_token})
        self.assert200(r.status_code)

        r = requests.get(host + INTER_BACKUP_FILE_URL_V2 + "?file=" + file_name,
                         stream=True,
                         headers={"Authorization": "token " + backup_token})
        self.assertEquals(r.text, input_text)

        r = requests.delete(host + INTER_BACKUP_FILE_URL_V2 + "?file=" + file_name,
                            headers={"Authorization": "token " + backup_token})
        self.assert200(r.status_code)
