...
from hive.util.constants import APP_ID, VAULT_ACCESS_R, HIVE_MODE_TEST, HIVE_MODE_DEV, \
    ...
    VAULT_BACKUP_SERVICE_MAX_STORAGE, INTER_BACKUP_SAVE_FINISH_URL_V2, INTER_BACKUP_RESTORE_FINISH_URL_V2, \
    INTER_BACKUP_SERVICE_URL_V2, INTER_BACKUP_FILE_LIST_URL_V2, INTER_BACKUP_FILE_URL_V2, CHUNK_SIZE, \
    INTER_BACKUP_PATCH_HASH_URL_V2, INTER_BACKUP_PATCH_DELTA_URL_V2, INTER_BACKUP_GENE_DELTA_URL_V2
...
from hive.main.v1.pre_proc import post_json_param_pre_proc, did_post_json_param_pre_proc, did_auth
...
class HiveBackup:
    ...
    def restore_vault_data(did):
        ...
        if info[VAULT_BACKUP_INFO_TYPE] == VAULT_BACKUP_INFO_TYPE_GOOGLE_DRIVE:
            ...
            ret = HiveBackup.restore_backup_finish(did, info[VAULT_BACKUP_INFO_DRIVE] + INTER_BACKUP_RESTORE_FINISH_URL_V2,
        ...
        if info[VAULT_BACKUP_INFO_TYPE] == VAULT_BACKUP_INFO_TYPE_GOOGLE_DRIVE:
            ...
            if not checksum_list:
                ...
                ret = HiveBackup.save_to_hive_node_finish(did,
                                                          info[VAULT_BACKUP_INFO_DRIVE] + INTER_BACKUP_SAVE_FINISH_URL_V2,
    ...
    def __proc_hive_node_param(self):
        ...
        data, err = self.get_backup_service(host + INTER_BACKUP_SERVICE_URL_V2, backup_token)
        if err:
            return None, None, err
        ...
        for info in file_put_list:
            ...
            try:
                with open(src_file, "br") as f:
                    f.seek(0)
                    url = host + INTER_BACKUP_FILE_URL_V2 + '?file=' + dst_file
                    r = requests.put(url,
                                     data=f,
    ...
    def get_unpatch_file_hash(file_name, host, token):
        get_file_hash_url = host + INTER_BACKUP_PATCH_HASH_URL_V2 + "?file=" + file_name
        try:
    ...
    def patch_remote_file(src_file_name, dst_file_name, host, token):
        ...
        post_delta_url = host + INTER_BACKUP_PATCH_DELTA_URL_V2 + "?file=" + dst_file_name
        try:
    ...
    def delete_files(file_delete_list, host, token):
        ...
        for name in file_delete_list:
            try:
                r = requests.delete(host + INTER_BACKUP_FILE_URL_V2 + "?file=" + name,
                                    headers={"Authorization": "token " + token})
            except Exception as e:
                logging.getLogger("HiveBackup").error(
    ...
    def save_to_hive_node_start(vault_folder, did, host, backup_token):
        ...
        try:
            r = requests.get(host + INTER_BACKUP_FILE_LIST_URL_V2,
                             headers={"Content-Type": "application/json", "Authorization": "token " + backup_token})
        except Exception as e:
            logging.getLogger("HiveBackup").error(
        ...
        for info in file_get_list:
            ...
            if not dst_file.parent.exists():
                if not create_full_path_dir(dst_file.parent):
                    ...
                    continue
            try:
                r = requests.get(host + INTER_BACKUP_FILE_URL_V2 + "?file=" + src_file,
                                 stream=True,
                                 headers={"Authorization": "token " + token})
    ...
    def patch_local_file(src_file_name, dst_file_name, host, token):
        ...
        try:
            with open(full_dst_file_name, 'rb') as open_file:
                ...
                for h in gene:
                    hashes += h
                r = requests.post(host + INTER_BACKUP_GENE_DELTA_URL_V2 + "?file=" + src_file_name,
                                  data=hashes,
                                  stream=True,
