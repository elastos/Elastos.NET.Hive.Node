...
from hive.main.v1.hive_backup import HiveBackup, VAULT_BACKUP_INFO_STATE, VAULT_BACKUP_MSG_SUCCESS, VAULT_BACKUP_INFO_MSG
from hive.util.constants import INTER_BACKUP_FILE_URL_V1, INTER_BACKUP_FILE_URL_V1
...
from .hive_auth_test import DIDApp, DApp
...
class Hive2NodeTest(unittest.TestCase):
    ...
    def test_4_inter_put_file(self):
        ...
        temp.seek(0)
        temp.name = 'temp.txt'
        url = self.host2 + INTER_BACKUP_FILE_URL_V1 + '?file=' + file_name
        r = requests.put(url,
                         data=temp,
                         headers={"Authorization": "token " + backup_token})
        self.assert200(r.status_code)

        r = requests.get(host + INTER_BACKUP_FILE_URL_V1 + "?file=" + file_name,
                         stream=True,
                         headers={"Authorization": "token " + backup_token})
        self.assertEquals(r.text, input_text)

        r = requests.delete(host + INTER_BACKUP_FILE_URL_V1 + "?file=" + file_name,
                            headers={"Authorization": "token " + backup_token})
        self.assert200(r.status_code)
